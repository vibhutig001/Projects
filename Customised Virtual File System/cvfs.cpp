...
//#include<io.h>
//user defined macro
#define MAXINODE 5//kiti file create kru shakato

#define READ 1//permission
#define WRITE 2//permission

#define MAXFILESIZE 100//max file size 100 byte chi tyar kru shakato//1024

#define REGULAR 1//txt c cpp(regular file)
#define SPECIAL 2//device driver
...
PINODE head = NULL;//shikshaacha dokyatli value
...
void man(char *name)
{
    ...
    else if(strcmp(name,"read")==0)
    {
        ...
        printf("Usage : create File_name No+Of_Bytes+To_Read\n");
    }
    ...
    {
        printf("Description : Used to chanfe file offset \n");
        printf("Usage : lseek File_Name ChangeInOffset StartPoint\n");
    }
    else if(strcmp(name,"rm")==0)
    {
        printf("Description : Used to the file \n");
...
void DisplayHelp()
{
    printf("ls : To list out of all files\n");
    printf("clear : To clear console\n");
    printf("open : To open the file\n");
    printf("close : To close the file\n");
    printf("closeall : To closed all opened files\n");
    printf("read : To read the contents from files\n");
    printf("write : To write contents into file\n");
    printf("exit : To terminate file system\n");
    printf("stat : To display information of file using name\n");
    printf("fstat : To diplay information of file using file descriptor\n");
...
void CreateDILB()//done
{
    int i=1;
    PINODE newn = NULL;
    PINODE temp = head;//global variable
...
int WriteFile(int fd, char *arr, int isize)
{
    ...
    // Check if writeoffset has reached the maximum file size
    if ((UFDTArr[fd].ptrfiletable->writeoffset) == MAXFILESIZE)
        return -2; // Potential issue here, 'return -2' is repeated

    // Check if the file is of type REGULAR
    if ((UFDTArr[fd].ptrfiletable->ptrinode->FileType) != REGULAR)
        return -3;

    // Copy the data to the file buffer at the write offset
    strncpy((UFDTArr[fd].ptrfiletable->ptrinode->Buffer) +
                (UFDTArr[fd].ptrfiletable->writeoffset),
            arr, isize);

    // Update the write offset and file actual size
    (UFDTArr[fd].ptrfiletable->writeoffset) +=
        isize; // Potential issue: Should check if writeoffset + isize exceeds MAXFILESIZE
    (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) +=
        isize; // Potential issue: Should check if FileActualSize + isize exceeds MAXFILESIZE
...
int main()
{
    ...
    {
        fflush(stdin);//sagal dhuun taka flush
        strcpy(str,"");//rekami houn jail

        printf("\n Marvellous VFS : >");

        fgets(str,80,stdin);//scanf("%[^'\n']s",str);//terminal mdhe aadvi line 80 aste

        count=sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);
    //4*80array
        if(count==1)//aka shabdachi command India
        {
            ...
            {
                system("cls");//OS specific in mac clear command
                continue;
            }
        ...
        else if(count==2)//don shabdachi command India is
        {
        ...
        else if(count==3)
        {
            if(strcmp(command[0],"create")==0)
            {
                ...
                if(ret==-4)
                    printf("ERROR : memory allocationfailure\n");
                continue;
            }
